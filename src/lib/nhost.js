...
export const VITE_NHOST_BACKEND_URL = import.meta.env.VITE_NHOST_BACKEND_URL;

/** @type {import("@nhost/nhost-js").NhostClientConstructorParams} */
export const config = {
...
nhost.auth.onAuthStateChanged((event, session) => {
  console.log(
    `auth state changed. State is now ${event} with session: `,
    session
  );
  if (event === "SIGNED_IN") {
    isSignedIn.set(true);
